...
try{
    ...
    $labtype = $_GET['labtype1'];
            ...
            $sql = "SELECT * FROM customer
                ...
                WHERE customer.customer_name LIKE '%$search%' AND
                    schedules.sched_date LIKE '%$date1%'  AND
                    schedules.laboratory LIKE '%$labtype%'";
    ...
    foreach ($res as $key => $value) {
        ...
        //echo '<td>'. $value['lab_name'] . '</td>';
            echo '<td class="center aligned"><a class="ui blue labeled icon button" title="Edit" href="http://localhost/dost/main/update/'.$value['sched_id'].'"><i class="plus icon"></i>View More Details ...</a></td>';
            echo '</tr>';
        }
            echo '</tbody>';
            echo '</table>';
